"""
Test suite for the short (<= 8 bytes) fingerprinting case.
"""
...
def test_umash_fp_short(seed, multipliers, key, data):
    """Compare umash_fp_short with two calls to the reference."""
    expected = [
    ...
    actual = C.umash_fp_short(params[0].oh, seed, block, n_bytes)
...
def test_umash_fp_short_matches_fprint(seed, multipliers, key, data):
    """umash_fp_short must agree with umash_fprint for short inputs."""
    n_bytes = len(data)
    block = FFI.new("char[]", n_bytes)
    FFI.memmove(block, data, n_bytes)
    params = FFI.new("struct umash_params[1]")
    ...
    direct = C.umash_fp_short(params[0].oh, seed, block, n_bytes)
